...
export default function SavedWords({wordObjects}) {
  ...
  return (
    ...
    {wordObjects.map((wordObject, index) => {
          const wordClasses = wordObject.wordClasses;
          const antonyms = wordObject.antonyms;
          const synonyms = wordObject.synonyms;
          const audioUrl = wordObject.pronounciationAudio;
          const pronounciation = wordObject.pronounciationText
          const word = wordObject.word;
          const sentence = wordObject.sentence 
  ...
  >
    <img src={play} />
              ...
              <div>
                ...
                {pronounciation ? <p className="text-sm italic mt-1">{pronounciation}</p> : <p className="text-[var(--soft-text-light)] text-xs italic">No pronounciation available</p>}
              </div>
              <div className="flex justify-between align-center mt-4">
                <p className="">{sentence}</p>
                {synonyms.length > 0 || antonyms.length > 0 ? <div className="flex gap-2">
                  {synonyms.length > 0 && <span className="font-bold text-xs text-[var(--primary-color-light)] bg-[var(--secondary-text-light)]/30 py-1 px-3 rounded-lg border border-[var(--secondary-text-light)]">
                    {synonyms?.[0]}
                  </span>}
                  { antonyms.length > 0 && <span className="font-bold text-xs text-[var(--antonym-light)] bg-[var(--antonym-light)]/30 py-1 px-3 rounded-lg border border-[var(--antonym-light)]">
                    {antonyms?.[0]}
                  </span>}
                </div> : <div className=" w-full flex items-center gap-2">  
                <div className="border border-[var(--border-light)] flex-1"></div>
                <p className="text-[var(--soft-text-light)] text-xs italic">No synonyms or antonyms available</p>
                <div className="border border-[var(--border-light)] flex-1"></div>
                </div> }
              </div>
            </section>
